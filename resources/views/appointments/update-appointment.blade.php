    ...
    <div class="row">
                    ...
                    <div class="d-flex flex-wrap align-items-center justify-content-between">
                        <div class="d-flex flex-wrap align-items-center">
                            <div class="profile-img position-relative me-3 mb-3 mb-lg-0">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($appointment->customer->first_name) }}&background=0D8ABC&color=fff"
                                    alt="User-Profile"
                            ...
                            <div class="d-flex flex-wrap align-items-center mb-3 mb-sm-0">
                                <h4 class="me-2 h4">{{ $appointment->customer->first_name }} {{
                                    $appointment->customer->last_name }}</h4>
                                <span class="text-capitalize"> {{ $appointment->customer->phone_number }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-lg-3">
            <div class="card">
                ...
                <div class="card-body">
                    <h5 class="cardTitle"></h5><strong>Appointment ID:</strong> <span>{{ $appointment->id }}</span>

                    {{-- <p class="cardText"><strong>Issue Source:</strong> {{$appointment->appointmentSources->name}}
                    </p>
                    <p class="cardText"><strong>Category:</strong>{{$appointment->appointmentCategories->name}}</p>
                    <p class="cardText"><strong>Status:</strong>{{$appointment->status->name}}</p>
                    <p class="cardText"><strong>Disposition:</strong>{{$appointment->appointmentDisposition->name}}</p>
                    <p class="cardText"><strong>Department:</strong>{{$appointment->appointmentDepartment->name ??
                        'None'}}</p>
                    <p class="cardText"><strong>Creator:</strong>{{$appointment->creator->username}}</p>
                    <p class="cardText"><strong>Assigned To:</strong>{{$appointment->user->username}}</p>
                    <p class="cardText" id="customerCreated"><strong>Created At:</strong>{{$appointment->created_at}}
                    </p> --}}
                </div>
            </div>
        </div>
                    ...
                    <div class="card">
                        <div class="form-wrapper">
                            <form method="post" id="appointmentForm"
                                action="{{ route('update.appointment', $appointment->id) }}">
                                <div class="form-group">
                                    {{-- @include('appointments.includes.assigned_and_status') --}}
                                </div>
                                <div class="form-group">
                    ...
                    <div class="iq-timeline0 m-0 d-flex align-items-center justify-content-between position-relative">
                        <ul class="list-inline p-0 m-0">
                            <li>
                                ...
                                <h6 class="float-left mb-1">Appointment Created</h6>
                                <small class="float-right mt-1">{{$appointment->created_at->format('d F Y')}}</small>
